...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    // return Math.cos((x1*y1+x2*y2)/(Math.sqrt(x1*x1+y1*y1)*Math.sqrt(x2*x2+y2*y2)));
...
function toNumber(value, def) {
    // if(typeof value=="number"){return value}else{if(
    //     typeof value=="string"){return parseInt(value)}else{return def}
    // }
    throw new Error('Not implemented');
}
